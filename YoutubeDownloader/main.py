...
class AsyncDownload(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.downloadTitleLabel['text'] = "You're now downloading: {}".format(
                yt.title)
            ...
            if self.user != None:
                videoHistoryThread = AsyncDatabaseOperations('videoHistory', {
                    'title': yt.title, 'length': yt.length, 'link': self.url.get(), 'videoHistory': self.videoHistory}, self.user[0])
...
class YoutubeDownloader:
    ...
    def loginPage(self):
        ...
        self.errorMessageLabel = self.errorMessageLabel = ttk.Label(
    ...
    def modifyAccountInfo(self):
        ...
        passwordEntry.grid(column=1, row=1, sticky=(E, W), pady=10)
        passwordValidationEntry.grid(column=1, row=2, sticky=(E, W), pady=10)
        modifyInfoButton.grid(column=0, columnspan=2,
                              row=3, sticky=(S), pady=10)
